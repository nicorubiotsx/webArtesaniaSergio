...
export default function DashboardPro() {
  ...
  // Filtrar ventas según rango seleccionado
  ...
  return (
    <main className="bg-stone-100 text-stone-900 min-h-screen py-16 px-6 md:px-12 max-w-7xl mx-auto">
      {/* Botón Volver */}
      <button
        ...
        <>
          ...
          <section className="grid gap-6">
            {filteredSales.map(sale => (
              <motion.div
                key={sale.id}
                className="bg-white rounded-2xl shadow-lg p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition hover:shadow-xl"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.3 }}
              >
